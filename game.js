...
function placeholderBackground(ctx, width, height, time) {
  ctx.save();
  ctx.fillStyle = '#1976b8';
  ctx.fillRect(0, 0, width, height);
  ctx.fillStyle = '#56b9e8';
...
function frame(now) {
  ...
  if (state === 'playing') {
    ...
    if (birdBottom >= CONFIG.canvasHeight - CONFIG.groundHeight || birdTop <= 0 || pipes.some((pipe) => {
      const overlapsX = birdX + CONFIG.birdSize / 2 > pipe.x && birdX - CONFIG.birdSize / 2 < pipe.x + CONFIG.pipeWidth;
      return overlapsX && (birdTop < pipe.gapTop || birdBottom > pipe.gapBottom);
    })) crash();
